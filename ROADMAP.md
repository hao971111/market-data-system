...
- [ ] 分段延迟：JSON parse、业务解析、callback、writer enqueue、disk write
...
1. 去掉 `outer["data"].dump()` + Parser 内部二次 parse
2. 增加 JSON parse / callback / writer enqueue 分段延迟
3. writer 队列从 `std::deque + mutex` 升级为 SPSC ring queue
4. cache 从 mutex ring buffer 升级为 seq-lock / lock-free snapshot
5. 进一步降低日志路径干扰
